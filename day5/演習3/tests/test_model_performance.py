...
def test_model_performance_stability():
    ...
    for metric, value in latest.items():
        ...
        assert degradation <= max_degradation, f"{metric}が前回より著しく低下しています: {value:.4f} < {prev_value:.4f} (差: {degradation:.4f})"
...
def test_feature_importance(load_model):
    ...
    assert feature_importances[indices[0]] >= 0.1, f"最も重要な特徴量の重要度が低すぎます: {feature_importances[indices[0]]:.4f}"
...
def test_prediction_distribution(load_model, prepare_test_data):
    ...
    print(f"予測確率の統計: 平均={mean_prob:.4f}, 標準偏差={std_prob:.4f}, 最小={min_prob:.4f}, 最大={max_prob:.4f}")
    
    # 予測分布が極端に偏っていないことを確認
    assert 0.2 <= mean_prob <= 0.8, f"予測確率の平均が極端です: {mean_prob:.4f}"
    assert std_prob >= 0.1, f"予測確率の標準偏差が小さすぎます: {std_prob:.4f}"
    assert max_prob - min_prob >= 0.5, f"予測確率の範囲が狭すぎます: {max_prob - min_prob:.4f}"